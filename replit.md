...
- **Liquid Glass Mode** (DEFAULT): Toggle between classic and liquid glass visual modes. Liquid is default. Persisted in localStorage key `cupgs-glass-mode`. Context: `GlassModeContext.tsx` (`useGlassMode()` hook). `LiquidGlassFilters.tsx` returns null (SVG filters removed). CSS uses `.liquid-glass` class on `<html>`. Architecture: CSS-only glass via `backdrop-filter: blur()` + `saturate()` directly on elements. No SVG filters, no pseudo-elements for distortion (all `content: none`). CSS variables: `--lg-blur`, `--lg-opacity`, `--lg-level`, `--lg-border`. Dialog protection: `.lg-dialog-open` class disables backdrop-filter on background elements when dialogs open.
- **Glass Settings Panel**: 4 sliders — Blur (0–40px, default 8px), Opacity (0–40%, default 6%), Liquid Level (0x–3x, default 1x), Border (0–100%, default 0%). Persisted in localStorage key `cupgs-glass-settings`. Component: `GlassSettingsPanel.tsx`. Reset button restores defaults.
