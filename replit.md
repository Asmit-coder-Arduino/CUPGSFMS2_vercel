...
- **Registration Number Department Detection**: Student login auto-detects department from BPUT registration number pattern (digits 3-4 map to branch code via `bputRegNo.ts`). Department is locked during feedback submission — students cannot manually change their branch.
- **Branch-wise Feedback Restriction**: Feedback windows can target specific departments. Server-side enforcement in `POST /api/feedback` validates an active window exists for the course's department before allowing submission. Home page also filters windows by student's department.
- **Sidebar Toggle**: Desktop sidebar can be collapsed to icon-only mode via toggle button at the bottom. Collapsed state shows only navigation icons with tooltips.
