...
export function GlassModeProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          level: typeof parsed.level === "number" ? parsed.level : DEFAULT_SETTINGS.level,
  ...
  useEffect(() => {
    ...
    html.style.setProperty("--lg-level", `${settings.level}`);
  ...
  const updateSettings = (partial: Partial<GlassSettings>) => {
    setSettings(prev => {
      ...
      next.level = Math.max(0, Math.min(3, next.level));
